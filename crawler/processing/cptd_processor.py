...
class CPTDProcessor:
    def __init__(self, config: Config):
        ...
        self.__basic_getter: Optional[BasicGetter] = None
    ...
    @property
    def _basic_getter(self) -> BasicGetter:
        if self.__basic_getter is None:
            self.__basic_getter = BasicGetter(
                general_helpers.get_rabbit_server_details(
                    self._config, self._config.RABBITMQ_CPTD_USERNAME, self._config.RABBITMQ_CPTD_PASSWORD
                )
            )

        return self.__basic_getter
    ...
    def _ensure_no_feedback_errors(self, message_uuids: list) -> None:
        ...
        while time() < t_end and len(unconfirmed_uuids) > 0:
            fetched_message = self._basic_getter.get_message(self._config.RABBITMQ_CPTD_FEEDBACK_QUEUE)
