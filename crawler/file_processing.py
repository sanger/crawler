...
class CentreFile:
    ...
    def filtered_row(self, row: CSVRow, line_number: int) -> ModifiedRow:
        ...
        for key in self.ACCEPTED_FIELDS:
            if key in row:
                ...
                modified_row[key] = row[key].strip() if type(row[key]) is str else row[key]
    ...
    def parse_and_format_row(
        ...
        modified_row[FIELD_CREATED_AT] = datetime.now(tz=timezone.utc)
        modified_row[FIELD_UPDATED_AT] = datetime.now(tz=timezone.utc)
        ...
        modified_row[FIELD_FILTERED_POSITIVE_TIMESTAMP] = datetime.now(tz=timezone.utc)
