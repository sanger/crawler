...
SQL_MLWH_UPDATE_MOST_RECENT_SAMPLE_COLUMNS = (
    f"UPDATE lighthouse_sample AS lsample,"
    f"("
    f"SELECT"
    f"    temp_lsample.id,"
    f"    IF(most_recent_samples.most_recent_id IS NULL, 0, 1) AS { MLWH_IS_CURRENT },"
    f"    IF(most_recent_samples.most_recent_id IS NULL, NULL, temp_lsample.rna_id) AS { MLWH_CURRENT_RNA_ID }"
    ...
    f") AS updated_data"
    f" SET lsample.`{ MLWH_IS_CURRENT }`=updated_data.`{ MLWH_IS_CURRENT }`, lsample.`{ MLWH_CURRENT_RNA_ID }`=updated_data.`{ MLWH_CURRENT_RNA_ID }`"
    f" WHERE  lsample.id=updated_data.id"
    f" AND lsample.rna_id IN %s;"
)
