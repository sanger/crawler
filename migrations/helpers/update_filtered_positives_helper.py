...
def update_dart_filtered_positive_fields(config: ModuleType, samples: List[Dict[str, Any]]) -> bool:
    ...
    try:
        ...
        for plate_barcode, samples_in_plate in groupby_transform(samples, lambda x: x[FIELD_PLATE_BARCODE], reducefunc=lambda x: list(x)):
            try:
                labware_class = labclass_by_centre_name[samples_in_plate[0][FIELD_SOURCE]]
                plate_state = add_dart_plate_if_doesnt_exist(cursor, plate_barcode, labware_class)  # type:ignore
                if plate_state == DART_STATE_PENDING:
                    for sample in samples_in_plate:
                        ...
                        if well_index is not None:
                            ...
                            set_dart_well_properties(cursor, plate_barcode, well_props, well_index)  # type:ignore
                        else:
