...
def add_timestamps_to_samples(db):
  ...
  try:
    ...
    print(f'Time after adding field to samples collection: {datetime.now()}')
    ...
    print(f'Time after adding index to samples collection: {datetime.now()}')
    ...
    print(f'Time after ordering collections: {datetime.now()}')
    ...
    for collection_name in collection_names_chrono_order:
      print(f'\n-- Starting processing collection: {collection_name} at {datetime.now()} --')
      ...
      print(f'Time after adding field to collection: {datetime.now()}')
      ...
      print(f'Time after querying field and building list: {datetime.now()}')
      ...
      for batch in range(num_batches):
        ...
        print(f'\n-- Starting batch {batch + 1} of {num_batches}: {datetime.now()} --')
        ...
        # )
        print(f'Time after querying based on new field and updating with timestamp: {datetime.now()}')
...
def extract_timestamp(collection_name):
  ...
  if m:
    ...
    timestamp_datetime = datetime.strptime(timestamp_string, "%y%m%d_%H%M")
  else:
    m = re.match(COLLECTION_NAME_FORMAT_2, collection_name) # e.g. samples_21052020_1510

    if m:
      timestamp_string = m.group(1)
      timestamp_datetime = datetime.strptime(timestamp_string, "%d%m%Y_%H%M")
    else:
