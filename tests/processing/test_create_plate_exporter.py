...
def test_export_data_adds_no_errors_to_the_message(subject, create_plate_message):
    subject.export_data()

    assert create_plate_message.has_errors is False


def test_export_data_puts_a_source_plate_in_mongo(subject, mongo_database):
    _, mongo_database = mongo_database
...
def test_export_data_sets_the_source_plate_uuid(subject, mongo_database):
    _, mongo_database = mongo_database
...
def test_export_data_puts_a_source_plate_in_mongo_only_once(subject, mongo_database, create_plate_message):
    _, mongo_database = mongo_database
...
def test_export_data_adds_an_error_when_source_plate_exists_for_another_lab_id(subject, create_plate_message):
